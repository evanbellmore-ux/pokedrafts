...
- **Choose a move** using its labelled selection control. The top summary shows that move's damage, supplied one-use KO chance and, when safe, a damage-only defender HP range. Selecting a move never applies damage to either build. Current HP and estimated remaining HP are separate.
- Use **Change Pokémon** or **Edit HP** at the top to open and focus the existing roster/manual selector or HP input. Blank current HP means full HP; invalid input pauses results instead of retaining stale damage. The numerical health summary remains visible while browsing on viewports at least 800px tall; shorter screens use normal scrolling.
- Use **Teams** to open **League and opponent** and choose an opponent. Your roster starts on the left and theirs on the right in **Pokémon and build settings**; **Change Pokémon** also takes you directly to those roster shortcuts. Click a roster Pokémon to make it active. Loading a league or opponent never overwrites the current builds.
...
League, Pokémon/build, field and coverage sections start collapsed. Their controls stay mounted, preserving raw input and session edits; collapsing a section never resets it. Errors remain visible by the move list with **Fix settings** or **Retry calculator**, and settings summaries flag errors even while closed. Team-loading failures have a separate retry path and do not disable manual calculations.
...
- The top **Defender HP remaining** range is `max(0, currentHP − maximumDamage)` through `max(0, currentHP − minimumDamage)`, from the latest unfiltered result and only if the move connects. Builds must be valid, bounds must be consistent nonnegative integers, and rolls must be a complete fixed/flat distribution. Explicit HP of zero is invalid input, not full HP; a projected zero is allowed but is not a simulated faint or a replacement KO probability.
...
- Selecting a move does not alter input HP or build-cache entries. Attacker HP remains its baseline; recoil, drain, healing, retaliation, secondary effects and later turns are not simulated. Loading, calculator errors, invalid settings and missing results immediately remove the numerical projection instead of showing a last-valid estimate.
