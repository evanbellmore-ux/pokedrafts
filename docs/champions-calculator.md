...
- Expand **Field conditions** for Singles/Doubles, weather, terrain, screens, Helping Hand and critical hits. Weather/terrain are explicit state, not automatically set by entry abilities. Do not apply Intimidate manually and through its entry checkbox simultaneously.
...
- The adapter compensates for two verified pinned-engine gaps: Gale Wings priority is supplied before priority-immunity checks, and effective Damp prevents explosive moves. The vendored engine's battle source is unchanged.
